...
* Unit testing via link:{uri-jenkins-pipeline-unit}[Jenkins Pipeline Unit] in the `test` suite
* Integration testing via link:{uri-jenkins-test-harness}[Jenkins Test Harness] (`JenkinsRule`) in the `integrationTest` suite
* Jenkins BOM auto-injected for the configured LTS line — plugin versions stay aligned by default
* `LocalLibraryRetriever` generated into the `integrationTest` source set — wires the local library into `JenkinsRule` tests without a network checkout
* Auto-registration of the shared library into the embedded Jenkins instance via a generated `SharedLibraryAutoRegistrar` — no manual `GlobalLibraries.get()` call required in tests
* Additional suites (JUnit Jupiter, Spock, Kotest, …) opt into full Jenkins wiring with a single `jenkinsTestRunnerSuite(suite)` call
...
|Jenkins LTS
|2.479.x, 2.492.x, 2.528.x, 2.541.x
|===
...
The Jenkins Maven repository is required for HPI/JPI plugin resolution.
...
----
[versions]
jenkins-bom = "5054.v620b_5d2b_d5e6"   # Renovate updates this

[libraries]
jenkins-bom = { module = "io.jenkins.tools.bom:bom-2.479.x", version.ref = "jenkins-bom" }
# Change module to bom-2.528.x (and update the version) to move to a newer LTS line.

[plugins]
jenkins-shared-library = { id = "com.mkobit.jenkins.pipelines.shared-library", version = "1.0.0" }
----
...
dependencies {
    // BOM aligns all default workflow plugin versions automatically.
    jenkinsPlugin(platform(libs.jenkins.bom))

    // Add extra Jenkins plugins by Maven coordinate; version is supplied by the BOM.
...
Register extra suites and opt them into full Jenkins wiring with `jenkinsTestRunnerSuite()`.
...
Version 1.0.0 is a clean break from the 0.x series.
