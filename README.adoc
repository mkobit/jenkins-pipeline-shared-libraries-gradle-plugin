...
* `LocalLibraryRetriever` and `SharedLibraryAutoRegistrar` generated into a dedicated `localLibraryRetriever` source set — auto-wired into all Jenkins test suites so tests can access the local library without a network checkout or manual `GlobalLibraries.get()` call
* Additional suites (JUnit Jupiter, Spock, Kotest, …) opt into full Jenkins wiring with a single `useJenkinsTestRunnerSuite(suite)` call
